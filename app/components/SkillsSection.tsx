...
export default function SkillsSection() {
  return (
    <section id="skills" className="relative py-10 bg-black">
      <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
        <div className="flex flex-col items-center justify-center text-center space-y-8 md:space-y-12">
          ...
          <motion.p initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.1 }} className="text-gray-300 max-w-3xl leading-relaxed text-lg md:text-xl mx-auto">
            A concise set of core frontend skills I use daily.
          </motion.p>
        </div>

        <motion.div className="mt-16 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-10 max-w-6xl mx-auto" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.15 }}>
          {skills.map((s) => {
            const Icon = s.icon;
            return (
              <motion.div key={s.name} whileHover={{ y: -12, scale: 1.05 }} transition={{ duration: 0.3, ease: 'easeOut' }} className="flex flex-col items-center justify-center gap-4 rounded-2xl bg-gradient-to-br from-purple-500/15 to-pink-500/8 border border-purple-500/30 p-6 hover:border-purple-500/70 hover:bg-purple-500/20 hover:shadow-xl hover:shadow-purple-500/40 transition-all duration-300 text-center group">
                <div className="p-3 rounded-2xl bg-gradient-to-br from-purple-600/80 to-pink-500/80 text-white group-hover:from-purple-500 group-hover:to-pink-400 transition-all duration-300">
